...
// Slideshow Component
const SlideshowImage = ({ images, title }: { images: string[], title: string }) => {
  ...
  return (
    <div className="relative w-full h-auto overflow-hidden group-hover:scale-[1.02] transition-transform duration-500 ease-out">
        ...
        <img src={images[0]} alt="spacer" className="invisible w-full h-auto" />
...
// Card Component
const ProjectCard = ({ shot }: { shot: DesignShot }) => {
    ...
    return (
        ...
        >
            ...
            >
                <SlideshowImage images={shot.images} title={shot.title} />
        ...
        <motion.div layout className="flex flex-wrap gap-3 mb-16">
          {categories.map((category) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full border-2 transition-all duration-300 font-display font-bold text-sm uppercase tracking-wide ${
          ...
          {featuredDesigns.length > 0 && (
            <motion.div layout className="mb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {featuredDesigns.map((shot) => (
                        <ProjectCard key={shot.id} shot={shot} />
                    ))}
                </div>
            </motion.div>
          )}
